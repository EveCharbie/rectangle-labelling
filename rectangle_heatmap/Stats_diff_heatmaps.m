...
% addpath(['/usr/local/MATLAB/R2019b/toolbox/fieldtrip-20220314'])
addpath('/home/user/Téléchargements/fieldtrip-20171231')
...
statcfg.numrandomization = 32 ; % devrait etre 1000
...
immatiade_neighbours = load('immediate_neighbours_rectangle.mat');
statcfg.neighbours = immatiade_neighbours.neighbours; %[] ;
...
for iSubjects = 1:length(filenames) % Novices
    ...
    ftAllFiles{1,i}.powspctrm(1,:,:) = squeeze(data2D) ;
    ftAllFiles{1,i}.dimord = 'chan_freq_time' ;
...
for iSubjects = 1:length(filenames) % Experts
    ...
    ftAllFiles{1,iSubjects+i}.powspctrm(1,:,:) =  squeeze(data2D);
    ftAllFiles{1,iSubjects+i}.dimord = 'chan_freq_time' ;
...
%     ftAllFiles{1,iSubjects+i}.freq = 1:214; % combien de points sur y
    ftAllFiles{1,iSubjects+i}.label = {'toto'} ;
    design(1,iSubjects+i) = iSubjects;
    design(2,iSubjects+i) = 2;
end
% design = [1:10 ; ones(1,5) ones(1,5)+1] ;
