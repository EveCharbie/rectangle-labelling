...
def image_treatment(small_image_last_frame, *args):
    global label_keys, number_of_points_to_label, points_labels

    sum_points = 0
    for key in label_keys:
        sum_points += np.sum(points_labels[key][:, frame_counter])
    ...
    if sum_points != 0:
        ...
        for i, key in enumerate(label_keys):
            last_frame_points_labels[:, i] = points_labels[key][:, frame_counter - 1]
        ...
        if len(unique_lines_index) > 0 or len(unique_borders_index) > 0:
            lines_next_frame_from_video_motion(small_image_last_frame)
        ...
        # check if next frame il n'y avait rien de labelé
...
def find_lines_to_search_for(last_frame_points_labels):
    ...
    borders_index = []
    for i in range(number_of_points_to_label):
        if active_points[frame_counter - 1, i] == True:
            x0 = int(last_frame_points_labels[0, i])
            y0 = int(last_frame_points_labels[1, i])
            for j in neighbors[i]:
                if j > i:
                    if active_points[frame_counter - 1, j] == True:
...
def lines_next_frame_from_video_motion(small_image_last_frame):
    ...
    grid_y, grid_x = np.mgrid[0:small_image_last_frame.shape[0]:1, 0:small_image_last_frame.shape[1]:1]
    p0 = np.stack((grid_x.flatten(), grid_y.flatten()), axis=1).astype(np.float32)

    p1, status, err = cv2.calcOpticalFlowPyrLK(small_image_last_frame, small_image, p0, None)

    movement_vector = p1 - p0

    # transform = estimateRigidTransform(small_image, small_image_last_frame,0)
    #
    ...
    # motion = 0
    #
    # small_image_blured = cv2.cvtColor(small_image, cv2.COLOR_BGR2GRAY)
    # small_image_blured = cv2.GaussianBlur(small_image_blured, (21, 21), 0)
    #
    # # Difference between static background
    # # and current frame(which is GaussianBlur)
    # diff_frame = cv2.absdiff(small_image_last_frame, small_image_blured)
    #
...
def find_line_pixels(small_image_last_frame):
...
def find_border_pixels(small_image_last_frame):
    ...
    # very_small_image = small_image_last_frame
...
def find_lines_through_pixels(lines_last_frame, borders_last_frame, lines_points_index, borders_points_index, unique_lines_index, unique_borders_index):
    ...
    image_gray_lines = find_line_pixels(small_image)
    image_gray_borders = find_border_pixels(small_image)

    # plt.figure()
    # plt.imshow(small_image_last_frame)
...
def find_pixel_close_to_lines(lines, image_gray):
    ...
    # plt.figure()
    # plt.imshow(small_image) # (image_gray)
...
def find_points_next_frame(lines_new_vert, lines_new_horz, lines_new_vert_index, lines_new_horz_index):
    ...
    # image_lines_points = small_image_last_frame.copy()
...
def distort_to_rectangle(points, lines_new_vert_index, lines_new_horz_index):
    ...
    def which_rectangle_is_visible(lines_new_vert_index, lines_new_horz_index):
        ...
        if i == len(rectangle_lines_definitions)-1 and sum_rectangle_lines != 4:
            ...
            position_corners_to_map = rectangle_points_position_definition[i, :]
            four_vertices_index = rectangle_points_definitions[i]

        return position_corners_to_map, four_vertices_index

    # Finding the four corner points and ordering them
    position_corners_to_map, four_vertices_index = which_rectangle_is_visible(lines_new_vert_index, lines_new_horz_index)
    ...
    if position_corners_to_map is None:
        ...
        image_to_distort = small_image_last_frame.copy()
...
############################### code beginning #######################################################################
global small_image, number_of_points_to_label, width_small, height_small, frame_counter, label_keys, points_labels, frames_clone
global ratio_image, Image_name, borders_points
...
rectangle_points_definitions[1, :] = np.array([0, 3, 15, 14])
rectangle_points_definitions[2, :] = np.array([4, 5, 19, 16])
rectangle_points_definitions[3, :] = np.array([1, 2, 19, 17])
rectangle_points_definitions[4, :] = np.array([0, 2, 18, 16])
rectangle_points_definitions[5, :] = np.array([4, 5, 15, 14])
rectangle_points_definitions[6, :] = np.array([0, 3, 5, 4])
rectangle_points_definitions[7, :] = np.array([14, 15, 19, 16])
rectangle_points_definitions[8, :] = np.array([6, 7, 13, 12])
rectangle_points_definitions[9, :] = np.array([0, 1, 17, 16])
rectangle_points_definitions[10, :] = np.array([2, 3, 19, 18])

rectangle_lines_definitions = np.zeros((11, 4))
rectangle_lines_definitions[0, :] = np.array([0, 6, 7, 11])
rectangle_lines_definitions[1, :] = np.array([0, 7, 11, 5])
rectangle_lines_definitions[2, :] = np.array([1, 7, 11, 6])
rectangle_lines_definitions[3, :] = np.array([0, 7, 10, 6])
rectangle_lines_definitions[4, :] = np.array([0, 8, 11, 6])
rectangle_lines_definitions[5, :] = np.array([1, 5, 7, 11])
rectangle_lines_definitions[6, :] = np.array([0, 7, 11, 1])
rectangle_lines_definitions[7, :] = np.array([5, 7, 11, 6])
rectangle_lines_definitions[8, :] = np.array([2, 4, 8, 10])
rectangle_lines_definitions[9, :] = np.array([0, 7, 8, 6])
rectangle_lines_definitions[10, :] = np.array([0, 10, 11, 6])
...
while playVideo == True:
    ...
    if key == ord(','):  # if `<` then go back
        ...
        small_image_last_frame = small_image
        frame_counter += 1
        cv2.setTrackbarPos(Trackbar_name, Image_name, frame_counter)
        image_clone = frames_clone[frame_counter]
        small_image = cv2.resize(image_clone, (int(round(width / ratio_image)), int(round(height / ratio_image))))
        small_image_gray = cv2.cvtColor(small_image, cv2.COLOR_BGR2GRAY)
        cv2.imshow(Image_name, small_image_gray)
        image_treatment(small_image_last_frame)
        print("Stop")
        draw_points_and_lines()
